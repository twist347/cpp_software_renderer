...
int main() {
    ...
    sr::Color brush = PALETTE[0];
    sr::f32 thickness = 4.f;

    sr::Vec2f prev_mouse = win.mouse_pos();
    ...
    while (win.is_open()) {
        ...
        if (lmb || rmb) {
            const sr::Color c = rmb ? BG : brush;
            const sr::i32 r = std::max(1, static_cast<sr::i32>(thickness * 0.5f));
            const sr::Vec2f from = was_drawing ? prev_mouse : cur;
            const sr::Vec2f delta = cur - from;
            const sr::f32 dist = delta.len();
            const sr::f32 step = static_cast<sr::f32>(r);
        ...
        if (win.key_pressed(sr::Key::LeftBracket)) {
            thickness = std::max(MIN_THICKNESS, thickness - 1.f);
        }
        if (win.key_pressed(sr::Key::RightBracket)) {
            thickness = std::min(MAX_THICKNESS, thickness + 1.f);
        }
